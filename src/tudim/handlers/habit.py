...
async def handle_habit_log(uow: UnitOfWork, user: domain.User, action: Action, raw_message: str) -> str:
    ...
    extracted = await extract_habit_log(text=action.raw_text, user=user, active_habits=active)
