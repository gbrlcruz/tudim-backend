...
async def handle_note(uow: UnitOfWork, user: domain.User, action: Action, raw_message: str) -> str:
    existing_tags = await uow.tags.list_names_for_user(user.id)
    extracted = await extract_note(text=action.raw_text, user=user, existing_tags=existing_tags)
