...
class User(Base):
    ...
    daily_count_reset_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)

    # double-confirmation for destructive commands
    pending_destructive_action: Mapped[str | None] = mapped_column(String, nullable=True)
    pending_destructive_expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class Reminder(Base):
    ...
    status: Mapped[str] = mapped_column(String, default="pending")  # pending|sending|sent|failed|cancelled
